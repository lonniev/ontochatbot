...
const Router = {
  ...
  init: function(app, gbot, passport) {
    ...
    app.post( '/firebase/trust',
      ( req, res ) =>
      {
        const onRedirectError = function( reason )
        {
          console.log( reason );
        }
        ...
        const firebaseRef = new Firebase("https://glaring-inferno-2964.firebaseio.com/");
          
        // identify the new user at our Firebase by its GitHub username
        const currentUser =
        {
        ...
        // create or confirm a user at Firebase
        firebaseRef.createUser( currentUser,
          function( reason, userInfo )
          {
            // and then authenticate with github
            firebaseRef.authWithOAuthPopup( "github", onRedirectError );
